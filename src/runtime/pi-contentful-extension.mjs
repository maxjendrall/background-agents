...
module.exports = function(pi) {
  ...
  reg("contentful_http_get", "Read-only GET against Contentful management/delivery/preview APIs for the configured staging environment. Path is relative to /spaces/{space}/environments/{env}.", { api: { type: "string", enum: ["management", "delivery", "preview"] }, path: { type: "string" }, query: { type: "object" }, saveAs: { type: "string" } }, ["path"], "http_get");
  ...
  reg("contentful_list_entries", "List Contentful entries from preview/delivery/management APIs. Defaults to preview/staging.", { api: { type: "string", enum: ["management", "delivery", "preview"] }, contentType: { type: "string" }, limit: { type: "number" }, skip: { type: "number" }, include: { type: "number" }, select: { type: "string" }, query: { type: "object" } }, [], "list_entries");
