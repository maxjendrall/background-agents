...
module.exports = function(pi) {
  ...
  reg("contentful_api_help", "Explain how to use Contentful staging tools, including allowed HTTP GET paths and examples. Call this before contentful_http_get if unsure.", {}, [], "api_help");
  reg("contentful_http_get", "Read-only GET against Contentful management/delivery/preview APIs for staging only. Prefer typed tools first. Path is relative to /spaces/{space}/environments/staging. Call contentful_api_help for paths/examples.", { api: { type: "string", enum: ["management", "delivery", "preview"] }, path: { type: "string" }, query: { type: "object" }, saveAs: { type: "string" } }, ["path"], "http_get");
