    ...
    toolkits({ config, job }) {
      ...
      return [toolKit({
        ...
        tools: {
          ...
          export_assets: hostTool({ description: "Export Figma nodes as images/assets to disk and return a small preview.", timeout: FIGMA_TIMEOUT, inputSchema: z.object({ fileKey: z.string().min(1), nodeIds: z.array(z.string()).min(1), format: z.enum(["png", "svg", "pdf", "jpg"]).optional(), scale: z.number().optional() }), execute: ({ fileKey, nodeIds, format, scale }) => figma.exportAssets(fileKey, nodeIds, format, scale) }),
