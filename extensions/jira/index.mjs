...
async function buildPrompt(config, body, batchedEvents) {
  ...
  const prompt = [
    ...
    `- Before finishing, add one Jira comment summarizing what changed, tests run, PR/status, and blockers.`,
    noExplicitInstructions ? `- This was started without explicit extra instructions. First investigate the ticket and relevant code. If anything is unclear, use the final Jira comment to ask a concrete clarification question. If it is clear, proceed with implementation and report results in the final Jira comment.` : "",
    ...
    `- If unclear, use the single end-of-turn jira_add_comment to ask a clarifying question and include your proposed plan.`,
    `- When done, use one end-of-turn jira_add_comment to report results.`,
  ].filter(Boolean).join("\n");
    ...
    toolkits({ config, job }) {
      ...
      return [toolKit({
        ...
        tools: {
          ...
          add_comment: hostTool({ description: "Add a comment to a Jira issue.", inputSchema: z.object({ issueKey: z.string().min(1), comment: z.string().min(1) }), execute: ({ issueKey, comment }) => jira.addComment(issueKey, comment) }),
